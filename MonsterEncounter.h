...
public:
    ...
    // health and damage the player regenorates and dose to monster.
    int playerMove3regenval = 4;
    int playerMove3dmgval = 4;


    MonsterEncounter(); // class
    ...
    void fourSecTimmer();// These next three scripts work together to genorate a visual delay between the players last move and the monsters next move to make the change between the turns more obvious.
    void WhenTimerRunsOut();
    void RemoveTime();
    void ClearScreen();// Convenient clear screen void.
